...
import 'FourthLevel.dart';
import 'LentilleDetails.dart';

class ThirdLevel extends StatefulWidget {
  @override
  _ThirdLevelState createState() => _ThirdLevelState();
}

class _ThirdLevelState extends State<ThirdLevel> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        body: FutureBuilder(
            future: Future.wait([
              DBProvider.db.getAllLentillesthirdlevel(
                  controller.path[0].toString(), controller.path[1].toString()),
              DBProvider.db.getAllTraitementsavecLentilles(),
              DBProvider.db.getLentilleNiveau2(
                controller.path[0],
                controller.path[1],
              )
            ]),
